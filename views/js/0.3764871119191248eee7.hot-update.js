	...
	var List = function (_React$Component3) {
	  ...
	  _createClass(List, [{
	    ...
	    value: function onKeyDown(event) {
	      if (event.key === "ArrowDown" && this.state.current_bar < this.props.data.length) {
	        event.preventDefault();
	        this.setState({
	          current_bar: this.state.current_bar + 1
	        });

	        if (this.state.current_bar > this.props.data.length / 2) window.scrollTo(0, 43 * Math.round(this.props.data.length / 2));
	        ...
	        if (this.state.current_bar < this.props.data.length / 2) window.scrollTo(0, 0);
	      }
	    }
	  }, {
	    key: "render",
